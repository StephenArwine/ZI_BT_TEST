...
int le_server(int(*callback)(int clientnode,int operation,int cticn,void* userData), void* userData,int timerds)
  {
    ...
    if(n >= 0)
...
#else
      if(callback != NULL)
        retval = callback(dp->node,op,cticn, userData);
      cbflag = 1;
#endif
      setkeymode(1);                
      ...
      for(dn = 0 ; devok(dn) != 0 && cbflag == 0 ; ++dn)
        ...
        if(dev[dn]->btletods > 0)
          ...
          if(dev[dn]->btletods == 0)
            {
            retval = callback(dev[dn]->node,LE_BTLETIMER,0,userData);
...
#else
        if(callback != NULL)
          retval = callback(localnode(),LE_TIMER,0,userData);
#endif
        setkeymode(1);
        tim0 = timems(TIM_RUN);
        }
...
#else
        if(callback != NULL)
          retval = callback(localnode(),LE_KEYPRESS,key,0);
#endif
        key = 0;
        }
      }  
